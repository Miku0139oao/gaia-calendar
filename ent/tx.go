...
func (tx *Tx) init() {
	...
	tx.GaiaSession = NewGaiaSessionClient(tx.config)
	tx.PasswordResetToken = NewPasswordResetTokenClient(tx.config)
	tx.ScheduleEntry = NewScheduleEntryClient(tx.config)
	tx.ScheduleSyncRun = NewScheduleSyncRunClient(tx.config)
...
// applies a query, for example: AppSession.QueryXXX(), the query will be executed
